...
# useful functions
def log(msg, name="log.txt",verbose=True): # Logging messages and errors | Appends <msg> to the log <name>, prints if verbose
        msg=str(msg)
...
class Chatbot():
        ...
        def log(self, msg, name="log.txt"): # Logging messages and errors | Appends <msg> to the log <name>, prints if self.verbose
                log(msg,name,verbose=self.verbose)

        def login(self, host="codegolf.stackexchange.com", hasCaptcha="false"): # Login to SE
                ...
                payload = {"email": email, "password": password, "isSignup":"false", "isLogin":"true","isPassword":"false","isAddLogin":"false","hasCaptcha":hasCaptcha,"ssrc":"head","submitButton":"Log in",
                           "fkey": fkey}
                ...
                if r.find('<a href="https://{}/users/logout"'.format(host))<0:
                        if 'Human verification' in r:
                                if hasCaptcha == "false":
                                  log('Caught by CAPTCHA, retrying in 30 seconds')
                                  time.sleep(30)
                                  self.login()
                                  return
                                else:
